...
public class Map {
    private static Petak<? extends Plant, ? extends Zombie>[][] MatriksPetak;
    int wave = 0;
    ...
    public static Petak<? extends Plant, ? extends Zombie>[][] getMatriksPetak() {
        return MatriksPetak;
    }
    ...
    public static Petak<? extends Plant, ? extends Zombie> getFromMatriksPetak(int row, int column) {
        ...
        if (spawnedZombies.size() < 10) {
            for (int i = 0; i < 6; i++) {
                if (random.nextDouble() < 0.3 && spawnedZombies.size() < 10) {
                    ...
                    Petak<? extends Plant, ? extends Zombie> tile = MatriksPetak[i][9];
                    Petak<? extends Plant, ? extends Zombie> spawnSite = MatriksPetak[i][10];
    ...
    public void moveZombies() {
        for (int i = 0; i < MatriksPetak.length; i++) {
            for (int j = MatriksPetak[i].length - 1; j > 0; j--) {
                Petak<? extends Plant, ? extends Zombie> petak = MatriksPetak[i][j];
                Petak<? extends Plant, ? extends Zombie> nextPetak = MatriksPetak[i][j - 1];
    ...
    public static void attackPlants() {
        for (int i = 0; i < 6; i++) {
            for (int j = 1; j < 10; j++) {
                Petak<? extends Plant, ? extends Zombie> currTile = getFromMatriksPetak(i, j);
    ...
    public void viewMap() {
        for (int i = 0; i < MatriksPetak.length; i++) {
            for (int j = 0; j < MatriksPetak[i].length; j++) {
                Petak<? extends Plant, ? extends Zombie> currentTile = MatriksPetak[i][j];
                ...
                if (zombieCount > 0) {
                    ...
                    if (currentTile.getListTanaman().get(0).getHealth() <= 0 || currentTile.getListTanaman().get(0).getPlantDie()) {
                        try {
    ...
    public void addPlantToTile(int row, int column, Plant plant) {
        Petak<? extends Plant, ? extends Zombie> tile = MatriksPetak[row - 1][column];
        try {
    ...
    public void removePlantFromTile(int row, int column) {
        Petak<? extends Plant, ? extends Zombie> tile = MatriksPetak[row][column];
        try {
            tile.removeTanaman();
